...
// import { type ECBasicOption, ECElementEvent, ElementEvent } from "echarts/types/dist/shared";
import { ref, Ref, onMounted, computed, type ComputedRef } from "vue";
...
const chartOption: ComputedRef<EChartsOption> = computed(function () {
  return {
    tooltip: {},
    geo: {
      tooltip: {
...
onMounted(function () {
  fetch("/Map_of_Iceland.svg")
    ...
    .then(async (svg) => {
      ...
      instance.getZr().on("click", handleAddClick);
      // await nextTick();
      // echarts.
...
function handleAddClick(params: ElementEvent) {
  ...
  const point = chartInstance.value?.convertFromPixel("geo", [
    params.offsetX,
    params.offsetY,
  ]);
...
function changeState(point: number[]) {
  ...
  for (let i = 0; i < data.value.length; i++) {
    if (
      Math.abs(data.value[i][0] - point[0]) < 40 &&
      Math.abs(data.value[i][1] - point[1]) < 40
    ) {
      index = i;
      break;
    }
  ...
  chartInstance.value?.setOption({series:{data: data.value}}, false, true);
